...
export class MenuScene extends Phaser.Scene {
  ...
  create(data: { tracks: Record<string, Track> }) {
    ...
    this.add.text(width / 2, height * 0.48, 'SPACE: SINGLE RACE     ENTER: 5-RACE SERIES WITH SHOP', { ...FONT, fontSize: '36px' }).setOrigin(0.5);
    ...
    const names = Object.keys(data.tracks);
    const single = () => this.scene.start('Race', { series: createSeries(names, 5, Date.now() >>> 0, 1), tracks: data.tracks });
