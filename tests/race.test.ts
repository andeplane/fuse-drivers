...
test('replay is deterministic and matches the recorded hash', () => {
  ...
  assert.equal(hash(a), '7230b8fd8b8637ca');
});
...
test('runner is frame-rate independent and bounds catch-up', () => {
  ...
  const b = createRaceRunner(track, 3);
  for (let i = 0; i < 60; i++) a.advance(1000 / 60, inputs);
  for (let i = 0; i < 30; i++) b.advance(1000 / 30, inputs);
  assert.equal(hash(a.state), hash(b.state));
  const c = createRaceRunner(track, 3);
  c.advance(TICK_MS * 50, inputs);
  assert.equal(c.state.tick, 5);
