...
- Displays receive snapshots every second tick (15 Hz) plus the events since the previous snapshot, and interpolate with the same `renderSnapshot` as single-player, one snapshot interval behind. Individual-device online play is the display page on a small screen with the controller overlaid.
