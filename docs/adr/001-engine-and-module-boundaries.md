...
- **Race runner.** `src/shared/runner.ts` exports `createRaceRunner(track, seed, stats, bots)` with `advance(elapsedMs, inputsBySlot) → { state, events }`. It owns the fixed **30 Hz** accumulator (at most five steps per call, backlog beyond that discarded so a backgrounded tab does not fast-forward a race), calls `botInput` for bot slots and holds bot memory, and concatenates the events of every step it performed. `RaceScene.update` calls `runner.advance(delta, inputs)` and nothing else from the sim. The M3 server calls the same function from a timer.
- **Render interpolation.** `src/client/render/interpolate.ts` exports `renderSnapshot(prev, next, alpha)`. Positions lerp; headings (radians in state; config values in degrees are converted once at config load) lerp by shortest arc; a truck whose `respawnedTick === next.tick` or that moved more than 200 u in one tick snaps to `next`. `alpha` comes from the render clock: single-player uses the accumulator remainder, M3 uses a render time held one snapshot interval behind the newest received tick. Spin-out is a sprite tween; the sim keeps heading fixed during spin-out. Truck sprites are one strict top-down image per color, nose up, downscaled with a Lanczos filter to a 256 px cell; the renderer rotates it to the interpolated heading, so turning is continuous (amended 2026-09-16: the 16 drawn directions of the tilted view snapped visibly and did not match each other, and the original game turns smoothly). Frames are never downscaled to world-unit size on disk; Phaser draws at the displayed size times device pixel ratio, so the chunky source pixels survive on 1080p and 4K alike. Nearest-neighbour downscaling of sprites is banned; it produced the noisy first preview.
- Scenes: `Boot`, `Menu`, `Race`, `Hud` (parallel, unscaled), `Shop` (M2), `Results`. The HUD reads snapshots; it never mutates them.
- The canvas is 1600×900 with `Scale.FIT` letterboxing. The race world is 1024×516 units (ADR 003) behind a fixed camera zoomed ×1.5625 so it fills the width below the HUD strip; menus and the HUD draw in canvas pixels (`config.screen`). Tiles are 32 units. *Amended 2026-09-15:* the world was 1600×900 units at zoom 1, which made the track unreadably small.
